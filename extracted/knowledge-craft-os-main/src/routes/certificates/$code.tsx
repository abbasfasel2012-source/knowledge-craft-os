    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-10">
      <Card className="w-full max-w-lg border-gold/40 shadow-[var(--shadow-gold)]">
        <CardContent className="flex flex-col items-center gap-6 p-8 text-center">
          ...
          <div>
            <p className="text-xs text-muted-foreground">شهادة إتمام</p>
            <h1 className="mt-1 text-xl font-bold">{user?.full_name ?? "متدرب"}</h1>
          </div>
          <div className="w-full border-t border-border pt-4">
            <p className="text-xs text-muted-foreground">أكمل بنجاح دورة</p>
            <p className="mt-1 text-lg font-semibold">{course?.title ?? "دورة تدريبية"}</p>
          </div>
