...
function AdminDashboard() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    enabled: user?.role === "owner" || user?.role === "instructor" || user?.role === "moderator",
    queryKey: ["admin-stats"],
    queryFn: async () => {
      const { data: coursesCount } = await supabase.from("courses").select("*", { count: "exact" });
      const { data: usersCount } = await supabase.from("profiles").select("*", { count: "exact" });
  ...
  if (!user || !["owner", "instructor", "moderator"].includes(user.role)) {
    return (
                ...
                >
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2 py-1 text-xs font-semibold ${
                      u.role === "owner" || u.role === "instructor" || u.role === "moderator"
