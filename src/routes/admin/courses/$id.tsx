...
function EditCourse() {
  ...
  const { data: lessons } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("lessons").select("*").eq("course_id", id).order("position");
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { toast.success("تم تحديث الدورة"); queryClient.invalidateQueries({ queryKey: ["admin-course"] }); },
    onError: (err: Error) => toast.error(err.message),
  });

  return (
    <div className="px-4 pt-6">
      <Link to="/admin/courses" className="mb-3 text-xs text-muted-foreground hover:text-foreground">← العودة للدورات</Link>
      <h1 className="mb-4 text-lg font-bold">تحرير: {course?.title}</h1>

      <Card className="mb-4 border-border">
        <CardHeader><CardTitle className="text-sm">معلومات الدورة</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-1.5"><Label>العنوان</Label><Input defaultValue={course?.title} onBlur={(e) => updateMutation.mutate({ title: e.target.value })} /></div>
          <div className="space-y-1.5"><Label>الملخص</Label><Input defaultValue={course?.summary} onBlur={(e) => updateMutation.mutate({ summary: e.target.value })} /></div>
          <div className="space-y-1.5"><Label>الوصف</Label><Textarea defaultValue={course?.description} onBlur={(e) => updateMutation.mutate({ description: e.target.value })} className="min-h-[80px]" /></div>
          <div className="space-y-1.5"><Label>الحالة</Label><Select defaultValue={course?.status} onValueChange={(v) => updateMutation.mutate({ status: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="draft">مسودة</SelectItem><SelectItem value="published">منشور</SelectItem><SelectItem value="archived">مؤرشف</SelectItem></SelectContent></Select></div>
          <div className="flex items-center gap-2"><Switch defaultChecked={course?.sequential} onCheckedChange={(v) => updateMutation.mutate({ sequential: v })} /><Label>تعلّم متسلسل</Label></div>
          <div className="flex items-center gap-2"><Switch defaultChecked={course?.certificate_enabled} onCheckedChange={(v) => updateMutation.mutate({ certificate_enabled: v })} /><Label>تفعيل الشهادات</Label></div>
        </CardContent>
      </Card>
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <Button onClick={() => setShowLessonForm(!showLessonForm)} size="sm" className="gold-gradient text-gold-foreground"><Plus className="h-4 w-4" /> درس جديد</Button>
        </div>

        {showLessonForm && <LessonForm courseId={id} onClose={() => setShowLessonForm(false)} onSaved={() => { setShowLessonForm(false); queryClient.invalidateQueries({ queryKey: ["admin-course-lessons"] }); }} />}
        ...
        <div className="space-y-2">
          ...
          {(!lessons || lessons.length === 0) && <p className="py-4 text-center text-sm text-muted-foreground">لا توجد دروس. أضف أول درس!</p>}
        </div>
      </div>
    </div>
  );
...
function LessonForm({ courseId, onClose, onSaved }: { courseId: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from("lessons").insert({
      course_id: courseId, title, type, content: content || null,
      video_url: videoUrl || null, pdf_url: pdfUrl || null, audio_url: audioUrl || null,
      script_text: scriptText || null, ai_context: aiContext || null, summary: summary || null,
      duration_minutes: duration, position: nextPos, is_preview: isPreview,
    });
    setSaving(false);
    if (error) { toast.error(error.message); return; }
  ...
  return (
    <Card className="mb-4 border-border">
      <CardHeader><CardTitle className="flex items-center justify-between text-sm">درس جديد <X className="h-4 w-4 cursor-pointer" onClick={onClose} /></CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <div className="space-y-1.5"><Label>عنوان الدرس</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="عنوان الدرس" /></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>المدة (دقائق)</Label><Input type="number" value={duration} onChange={(e) => setDuration(Number(e.target.value))} /></div>
          <div className="flex items-end gap-2 pb-1.5"><Switch checked={isPreview} onCheckedChange={setIsPreview} /><Label>معاينة مجانية</Label></div>
        </div>
        <Button onClick={handleSave} disabled={saving || !title.trim()} className="w-full gold-gradient text-gold-foreground">{saving ? "جارٍ الحفظ..." : "إضافة الدرس"}</Button>
      </CardContent>
    </Card>
  );
}
