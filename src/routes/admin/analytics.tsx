  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      enrollments.data?.forEach((e) => {
        const c = e.course as unknown as { title: string } | null;
        if (c) courseEnrollments.set(c.title, (courseEnrollments.get(c.title) ?? 0) + 1);
      });
      const chartData = Array.from(courseEnrollments.entries())
      ...
      const avgQuizScore =
        quizzes.data && quizzes.data.length > 0
          ? Math.round(
              quizzes.data.reduce(
                (s, q) => s + (q.max_score > 0 ? (q.score / q.max_score) * 100 : 0),
                0,
              ) / quizzes.data.length,
            )
          : 0;
      ...
      const statusData = [
        {
          name: "منشور",
          value: courses.data?.filter((c) => c.status === "published").length ?? 0,
          color: "oklch(0.75 0.125 85)",
        },
        {
          name: "مسودة",
          value: courses.data?.filter((c) => c.status === "draft").length ?? 0,
          color: "oklch(0.52 0.018 75)",
        },
        {
          name: "مؤرشف",
          value: courses.data?.filter((c) => c.status === "archived").length ?? 0,
          color: "oklch(0.3 0.02 70)",
        },
      ];
    ...
    <div className="px-4 pt-6">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3">
        <Card className="border-border">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-gold">{stats?.avgQuizScore ?? 0}%</p>
            <p className="text-[10px] text-muted-foreground">متوسط درجات الاختبارات</p>
          </CardContent>
        </Card>
        <Card className="border-border">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-gold">{stats?.avgRating ?? "—"}</p>
            <p className="text-[10px] text-muted-foreground">متوسط التقييمات</p>
          </CardContent>
        </Card>
      </div>
      ...
      {stats && stats.chartData.length > 0 && (
        <Card className="mb-4 border-border">
          <CardHeader>
            <CardTitle className="text-sm">التسجيلات حسب الدورة</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {stats && stats.statusData.some((s) => s.value > 0) && (
        <Card className="border-border">
          <CardHeader>
            <CardTitle className="text-sm">حالة الدورات</CardTitle>
          </CardHeader>
          <CardContent>
